    ...
    public static DailySchedule BuildFromTasks(List<SimTask> tasks, Dictionary<int, Address> addresses, MapConfig config)
    {
        ...
        for (int i = 0; i < blocks.Count; i++)
        {
            ...
            if (currentAddressId != prevAddressId && currentAddressId.HasValue && prevAddressId.HasValue)
            {
                ...
                // Compute block duration (accounting for midnight wrap)
                var blockDuration = blockEnd >= blockStart
                    ? blockEnd - blockStart
                    : 1440 - blockStart + blockEnd;

                // Cap travel at the block duration so it doesn't overflow into the next block
                travelMinutes = Math.Min(travelMinutes, blockDuration);
            ...
            // Only add the activity entry if travel didn't consume the entire block
