...
- **Git commands:** Run each git command as a separate Bash call — never chain with `&&` or `;`. Make one call, and then the other. NEVER use `git -C`, because this also breaks permissioning. Just call `git`.
