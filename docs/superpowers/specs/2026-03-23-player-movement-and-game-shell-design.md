...
The current SimulationDebug scene becomes a content view (`scenes/city/CityView.tscn`) stripped of sidebar, clock, time controls, and debug panel (those now live in GameShell).

CityView.tscn contains only the map content:
...
### Right-Click Context Menus on City Map

**Address icons** — existing behavior plus a new item:
- **"Go here"** — begins player travel to that address.
- **"Add to Evidence Board"** — existing behavior (disabled if already added).

**NPC/player dots** — preserved as-is:
- **"Add to Evidence Board"** — existing behavior (disabled if already added).
...
Add to the Player entity:
- `CurrentPosition` (Vector2) — initialized to the player's home address position at creation time. Interpolated each frame during travel.
- `TravelInfo` (nullable) — reuses the same `TravelInfo` class that NPCs use (`FromPosition`, `ToPosition`, `DepartureTime`, `ArrivalTime`, `FromAddressId`, `ToAddressId`). Null when not traveling.
- `Id` (int) — generated via `SimulationState.GenerateEntityId()` at creation, same as NPCs. This ID is used for EventJournal logging so the replay system can track the player alongside NPCs.
...
When interpolation progress reaches 1.0 (handled by SimulationManager):
1. `Player.CurrentAddressId` updates to destination
2. `Player.TravelInfo` = null
...
Player position interpolation runs in `SimulationManager._Process()`, alongside NPC updates. This ensures the player continues moving even when viewing the Address scene (where CityView is not active). SimulationManager checks if the player has active TravelInfo and interpolates `CurrentPosition` each frame, handling arrival (clearing TravelInfo, setting CurrentAddressId, logging ArrivedAtAddress).

### No Schedule System

The player has no goal/schedule pipeline. Travel is driven by direct input: either idle at an address, or traveling between two points. Input triggers (right-click "Go here") set TravelInfo on the Player; SimulationManager handles the per-frame interpolation and arrival.

## 5. Address Content View

### Scene: `scenes/address/AddressView.tscn`
...
GameShell's active content view state is stored on GameManager (the autoload) so it survives the full scene change. On return from the Evidence Board, GameShell reads this state and loads the correct content view.
